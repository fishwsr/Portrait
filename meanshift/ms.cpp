...
tree_struct *MeanShift::BuildKDTree(tree_struct *subset, int length, int d, tree_struct* parent)
{
...
void MeanShift::QuickMedian(tree_struct *arr, int left, int right, int d)
{
	unsigned long k;
	unsigned long n;
	float* a;
...
void MeanShift::uniformSearch(tree_struct *gt, int gd, double *Mh_ptr, double *yk_ptr)
{
	tree_struct* c_t;
	int c_d;
	int i;
...
void MeanShift::generalSearch(tree_struct *gt, int gd, double *Mh_ptr, double *yk_ptr)
{
	tree_struct* c_t;
	int c_d;
	int i;
...
//-----------------------------hw add------------------------
void MeanShift::GetInitialLUVData(float *outData,int totalPoints_L,int dim_N)
{	//get the image data in LUV space
	// L is total Point of the image, dim: color image--3; gray--1
	// you should assure you have newed a L*N memory before you call this function
	//------------------------------------------------------------------------------

	assert(totalPoints_L == L);// the length is right
	assert(dim_N==N) ;// that must the same .
	for(int i=0;i<L*N;i++)
	{
		outData[i]=data[i];
	}
