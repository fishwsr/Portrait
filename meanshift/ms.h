 ...
 private:
  ...
  //========================
   ...
   void	ResetInput		( void );						// de-allocate memory for and re-initialize input data structure
	...
	////////Data Search Tree/////////
   tree_struct  *BuildKDTree (tree_struct*,  int, int, tree_struct* );       // Builds a kd tree given a subset of points initialized
                                                        // at depth 0 (dimension 0) (for Tree Structure)

   void  QuickMedian (tree_struct*, int, int, int );           // Finds the median tree in a forest of trees using
   ...
   void uniformSearch (tree_struct*, int, double*, double*);     // uses kdbst to perform range search on input data,
                                                        // computing the weighted sum of these points using
                                                        // a uniform kernel and storing the result into Mh
                                                        // (called by uniformMSVector)

   void generalSearch (tree_struct*, int, double*, double*);     // uses kdbst to perform range search on input data,
                                                        // computing the weighted sum of these points using
  ...
  //=============================
   ...
   //##########################################
	...
	tree_struct			*root;								// root of kdBST used to store input

	tree_struct			*forest;							// memory allocated for tree nodes
   ...
   //##########################################
	...
	//-----------------hw add-----------------------
	public:
	//int GetTotalPoints(){return L;} // the total points count output
				//we write it in the msImageProcessor
	void GetInitialLUVData(float *outDataint ,int totalPoints_L,int dim_N);//µÃµ½image data in LUV space
