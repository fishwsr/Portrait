...
{///如果regionList作为Boundary输出，则L相当于TotalBoundaryCount，而N不变
	///maxRegions 相当于最大的Region数，等于RegionCount。
...
{
	///有两种用处：
	///将一些包含在region i中的点添加到Region i中，regionList可以是每一个Region所包含的pixels
	///也可以是每一个Region所包含的边界点。
	///将点的个数存入Region中的数据。freeBlockLoc对应Region存储在indexTable中的首位置。
	...
	//block
	freeBlockLoc	+= pointCount; ///实际上表示的是indexTable中下一个Region的piexel的index的存储点的开始位置
