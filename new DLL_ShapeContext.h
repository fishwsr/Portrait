...
public:
	...
	void AffineTransform( double dSrcX, double dSrcY, double & dTarX, double & dTarY ) const;
	...
	double   Create( vector<double> vecSrcX, vector<double> vecSrcY, vector<double> vecTarX, 
		vector<double> vecTarY, bool bCompEnergy = false);
	double WeightX(int index) { return m_vecWeightX[index]; }
	double WeightY(int index) { return m_vecWeightY[index]; }
	double CoefX(int index) { return m_vecCoefX[index]; }
	double CoefY(int index) { return m_vecCoefY[index]; }
	int Size() { return m_vecWeightX.size(); }  // Assuming all vectors in this class have the same size...

	//Functions
	// To estimate the 2D rotation angle with Similarity Transform Assumption
	static double  RotationEstimate(vector<double> vSrcX, vector<double> vSrcY, vector<double> vTarX, vector<double> vTarY); 
	static double  RotationEstimate(vector<double> vSrcX, vector<double> vSrcY, CMyTPS& tps);
...
public:
	...
	//Initialize the source, target and parameters.  Coarse alignment flag will run the 
	// coarse alignment algorithm if true.
	bool Initialization(vector<double>& vSrcX, vector<double>& vSrcY, vector<double>& vTarX, 
		vector<double>& vTarY, CSCParam& param, bool coarse_alignment = true);

	void CoarseAlignment();  // Will try to align major shape moments of target and source before matching.
	...
	const vector<int>& GetAssignment() const { return m_vAssignment; }

	void GetMapping(int* pMapping);
	const vector<int>& GetMapping() const { return m_vMapping; } 

	int GetAssignmentSize() { return m_vAssignment.size(); }
	CMyTPS* GetTPS(){return &m_tps;};

	//Get energy vectors. Recording energy of each loop.
	const vector<double>& GetShapeContextEnergy() const {return m_vSCEnergy;};
	const vector<double>& GetAffineEnergy() const {return m_vAffineEnergy;};
	const vector<double>& GetTPSEnergy() const {return m_vTPSEnergy;};
...
public:
	...
	void SetUpdateData(const vector<double> vCurX, const vector<double> vCurY, const vector<int> vAssignment,
		               const vector<double> vAffineE, const vector<double> vSCE, const vector<double> vTPSE,
					   const int _nLoop, CMyTPS& tps);
	void SetUpdateAssign(const vector<int> vMapping);
...
protected:
	...
	//Energies
	vector<double> m_vSCEnergy;
	vector<double> m_vAffineEnergy;
	vector<double> m_vTPSEnergy;
