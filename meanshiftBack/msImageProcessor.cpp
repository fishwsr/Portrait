...
void msImageProcessor::FuseRegions(float sigmaS, int minRegion)
{
	...
	//DestroyRAM();         //modify by hongwei for we get the raList from the segmentation
...
void msImageProcessor::Segment(int sigmaS, float sigmaR, int minRegion, SpeedUpLevel speedUpLevel)
{
	...
	for(i = 0; i < L; i++)
	{
		...
		for(j = 0; j < N; j++)
		{
				msRawData[N*i+j] = modes[N*label+j];   /// model[label] is an N dimentional vector of the max density of distribution 
		} /// msRawData= < x11,...x1N,......,xL1,...xLN>
	}

	//----------hw add--------
	beSegmentCompleted= true;
	//========================
...
{//raList 是一个大小为regionCount的RaList链表。每一个对应一个region，然后此节点是以自己为首的Ralist，它之后
	//就是它的邻接region。按照label由小到大排列。
...
void msImageProcessor::DefineBoundaries( void )
{
	...
	for(i = 0; i < width; i++)
	{
			...
			boundaryCount[label]++; //对应每个label的边界pixel数
			totalBoundaryCount++;   
	}
	...
	for(i = 0; i < regionCount; i++)
	{
		boundaryIndex[i]	= counter;	/// 存储列表中region i的边界点的存储开始位置。
		counter			   += boundaryCount[i];   //boundaryCount[i] store the boundary pixels of region i $
	}
	...
	for(i = 0; i < L; i++)
	{
		...
		{
			boundaryBuffer[boundaryIndex[label]] = i;  ///逐步开始存储边界点的index
			boundaryIndex[label]++;
		}
	}
	...
	if(regionList)	delete regionList; ///这
	...
	for(i = 0; i < regionCount; i++)
	{
		regionList->AddRegion(i, boundaryCount[i], &boundaryBuffer[counter]);
								///对于每个region i，存储边界点的index。
		counter += boundaryCount[i];  ///更新下一个边界点序列的开始位置
	}
...
void msImageProcessor::DestroyOutput( void )
{
	...
	//-------------add by hongwei for getting the raList from segmentaion------
	this->DestroyRAM();
	//=========================================================================
               ...
               diff += el*el;
				...
				if (diff < speedThreshold)
				{
					...
					} else
					{
						...
						// the mode has already been associated with
						// another mode, therefore associate this one
						// mode and the modes in the point list with
						// the mode associated with data[basin_i]...
...
//------------------hw add-----------------------------
void msImageProcessor::OutputLabelToFile(int L)
{	// out put labels to file 
	std::ofstream labelOut("labelout.txt");
	for(int i=0;i<L;i++)
	{
		labelOut<<labels[i]<<" ";
	} 
	labelOut<<"regionCount="<<regionCount<<std::endl;
}

//void msImageProcessor::GetIndexRegionTable(int *idxTable, int length)
//{	//get indexTable: stored pixelIndex by region area
//
//	assert( length == L);// the length should match.
//	// you should get the right length=L out of the class msImageProcessor
//	// use GetTotalPoints()
//
//	for(int i=0;i<L;i++)
//	{
//		idxTable[i]=	indexTable[i];
//	}
//
//}
//
//void msImageProcessor::GetregionList(RegionList *regList, int length)
//{		//get regionList, for we get the REGIONs 
//	
//	assert(length  == regionCount) ;// you should get regionCount and put it as parameter.
//	for(int i=0;i<regionCount;i++)
//	{
//		regList[i]= regionList[i];
//	}    
//}

void msImageProcessor::GetLabelArray(int * labelArray,int length)
{	//get the labels[L]
	assert( length == L);//the length is the total points count.
