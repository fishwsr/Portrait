  ...
  //=============================
   ...
   //##########################################
	...
	int				*labels;				// assigns a label to each data point associating it to
											...
											// mode modes[l])
											//labels[L]指定每个点的label,不同区域都有不同的label
											//所以可以用于标定region，相当于regionID
   ...
   //------------------hw add------------------------
	private:
		bool beSegmentCompleted;
	public:
		bool IsSegmentCompleted(){return beSegmentCompleted;}

		int GetRegionCount(void){return regionCount;} //相当于得到了label的数量。

		RAList* Get_raList(void) {return raList;}	//get raList for initial nodeMap and edgeMap of SWGraph
		//raList 相当于一种紧凑的RAM，存储每一个region的邻接表。

		void OutputLabelToFile(int L) ; // out put labels to file 

		int GetTotalPoints(){return L;} ;// the total points count output

		//void GetIndexRegionTable(int *idxTable, int length) ;//get indexTable: stored pixelIndex by region area

		//void GetregionList(RegionList *regList, int length); //get regionList, for we get the REGIONs 

		void GetLabelArray(int * labelArray,int length); //get the labels[L]
